...
type Language = "zh" | "en";
type AssessmentMode = "project" | "task" | "agent" | "agency";
...
type AgentResponse = {
  ...
  orchestration?: {
    ...
    task_class: string;
    team_version: string;
    consensus: { agreement: number; spread: { minimum: number; maximum: number }; method: string };
    experts: NonNullable<Run["agencyReview"]>["experts"];
  };
  trace?: {
    stages: string[];
...
type IntelligenceSummary = {
  ...
  leaderboard: {
    ...
    profiles?: Array<{ name: string; resolved: number; success_rate: number; brier: number; calibration_score: number }>;
    specialists?: Array<{ name: string; profile_id: string; resolved: number; success_rate: number; brier: number; raw_brier: number; calibrated_brier: number; calibration_lift: number; calibration_score: number }>;
  };
...
const copy = {
  zh: {
    ...
    agencyModeHint: "由 3 位专业 Agent 独立判断并公开分歧",
    taskBrief: "任务说明",
    ...
    fallbackAuth: "DeepSeek 鉴权失败，请检查本地 API Key 是否有效；本次使用本地降级结果。",
    fallbackRate: "DeepSeek 请求达到额度或频率限制；本次使用本地降级结果。",
    fallbackTimeout: "DeepSeek 响应超时；本次使用本地降级结果。",
    fallbackOutput: "DeepSeek 返回内容不符合评估协议；本次使用本地降级结果。",
    agentReady: "本次结果由 Preflight Agent 生成。",
    agencyReady: "Agency 专家组已完成独立会审并封存共识。",
    agencyCouncil: "AGENCY 专家会审",
    agencyAgreement: "共识度",
    agencySpread: "预测区间",
    agencyMethod: "合成方法",
    agencyBoundary: "专家意见是结构化判断，不包含或保存隐藏思维链。最终结果仍需 Runner 以测试、构建和 Diff 结算。",
  ...
  en: {
    ...
    agencyModeHint: "Three specialist agents judge independently and expose disagreement",
    ...
    fallbackAuth: "DeepSeek authentication failed. Check the local API key; this run used the local fallback.",
    fallbackRate: "DeepSeek hit a quota or rate limit; this run used the local fallback.",
    fallbackTimeout: "DeepSeek timed out; this run used the local fallback.",
    fallbackOutput: "DeepSeek returned an invalid assessment payload; this run used the local fallback.",
    agentReady: "This result was generated by the Preflight Agent.",
    agencyReady: "The Agency council completed independent reviews and sealed a consensus.",
    agencyCouncil: "AGENCY EXPERT COUNCIL",
    agencyAgreement: "AGREEMENT",
    agencySpread: "FORECAST RANGE",
    agencyMethod: "AGGREGATION",
    agencyBoundary: "Expert opinions are structured judgments; hidden chain-of-thought is neither requested nor stored. The Runner must still settle the result with tests, build, and Diff.",
...
function mapAgentAssessment(response: AgentResponse, task: string, repo: string, language: Language): Run {
  ...
  return {
    ...
    agencyReview: response.orchestration ? {
      strategy: response.orchestration.strategy,
      taskClass: response.orchestration.task_class,
      teamVersion: response.orchestration.team_version,
      agreement: response.orchestration.consensus.agreement,
      spread: response.orchestration.consensus.spread,
      method: response.orchestration.consensus.method,
      experts: response.orchestration.experts,
    } : undefined,
  };
}
...
export function PreflightApp() {
  ...
  async function runPreflight(event: FormEvent) {
    ...
    let assessment: Run;
    try {
      const request = await fetch(mode === "agency" ? "/api/agency" : "/api/preflight", {
        method: "POST",
      ...
      setNotice(`${mode === "agency" ? t.agencyReady : t.agentReady} ${response.provider ? `${response.provider.toUpperCase()} · ` : ""}${response.model || ""}`.trim());
    } catch (error) {
      assessment = createLocalAssessment(task, repo, language, mode);
      const code = error instanceof Error ? error.message : "AGENT_UNAVAILABLE";
      setNotice(code === "AGENT_AUTH_FAILED" ? t.fallbackAuth
        : code === "AGENT_RATE_LIMITED" ? t.fallbackRate
          : code === "AGENT_TIMEOUT" ? t.fallbackTimeout
            : code === "AGENT_INVALID_OUTPUT" ? t.fallbackOutput
              : t.fallback);
    }
      ...
      <section className="workspace" aria-label="Agent preflight workspace">
        <form className="task-panel" onSubmit={runPreflight}>
          ...
          <div className="mode-switch" role="group" aria-label={t.assessmentMode}>
            ...
            <button type="button" className={mode === "agency" ? "active" : ""} onClick={() => setMode("agency")}>
              <strong>{t.agencyMode}</strong><span>{t.agencyModeHint}</span>
            </button>
          </div>
        ...
        <article className={`result-panel risk-${activeRun.risk.toLowerCase()}`} aria-live="polite">
          ...
          {activeRun.agencyReview && (
            <section className="agency-council">
              <div className="agency-council-head">
                <div><span>{t.agencyCouncil}</span><strong>{activeRun.agencyReview.taskClass.replaceAll("_", " ")}</strong></div>
                <div><span>{t.agencyAgreement}</span><strong>{activeRun.agencyReview.agreement}%</strong></div>
                <div><span>{t.agencySpread}</span><strong>{activeRun.agencyReview.spread.minimum}–{activeRun.agencyReview.spread.maximum}%</strong></div>
                <div><span>{t.agencyMethod}</span><strong>{activeRun.agencyReview.method}</strong></div>
              </div>
              <div className="agency-experts">
                {activeRun.agencyReview.experts.map((expert) => (
                  <article key={expert.profile_id}>
                    <header><div><span>{expert.profile_name}</span><small>{expert.profile_id}</small></div><b>{expert.probability}%</b></header>
                    <p>{expert.verdict}</p>
                    <div className="expert-meta"><span>{expert.route}</span><span>{expert.risk}</span><span>{expert.confidence}</span></div>
                    <ul>{expert.findings.slice(0, 3).map((finding) => <li key={finding}>{finding}</li>)}</ul>
                    {expert.veto_reason && <em>{expert.veto_reason}</em>}
                  </article>
                ))}
              </div>
              <p className="agency-boundary">{t.agencyBoundary}</p>
            </section>
          )}
              ...
              <>
                <section className="intelligence-block">
                  ...
                  <div className="leaderboard-grid">
                    {[...(intelligence.leaderboard.specialists || []), ...intelligence.leaderboard.models, ...intelligence.leaderboard.runners].slice(0, 12).map((entry, index) => (
                      <div key={`${entry.name}-${entry.resolved}-${index}`}><strong>{entry.name}</strong><span>{entry.resolved} {t.resolved}</span><span>{entry.success_rate}% {t.successRate}</span><b>{entry.calibration_score}/100</b></div>
                    ))}
                  </div>
                </section>
                <section className="intelligence-block">
